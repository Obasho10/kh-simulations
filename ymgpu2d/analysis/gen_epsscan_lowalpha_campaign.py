#!/usr/bin/env python3
"""Low-alpha extension of the EPS-scan v2 campaign (2026-07-20). Same Grid-B
magnitude-check + peak-location-cascade design as gen_epsscan_v2_campaign.py,
but reading sweep/eps_collapse_gridB_lowalpha.csv (alpha in {0.2,0.25,0.3,0.4}
instead of the original {0.5,1.5,3,6,10}) and spread across all 7 currently-
free streams (t126,t133,t136,t140,abi0,abi1,abi2) instead of the original 3
(abi was mid an unrelated campaign when epsscan_v2 ran; now free).

run_tag='epsscanlowalpha' (not 'epsscanv2') keeps these runs' output dirs
distinguishable from the original campaign's, so existing glob patterns in
measure_epsscan_v2_accuracy.py aren't accidentally fed low-alpha data.

Output: scripts/epsscan_lowalpha_{t126,t133,t136,t140,abi}.sh +
        sweep/epsscan_lowalpha_manifest.csv
Launch each single-node script with:
  ssh -f <node> "cd <wdir> && nohup bash scripts/epsscan_lowalpha_<node>.sh > logs/epsscan_lowalpha_<node>.log 2>&1 &"
"""
import os, sys, math
import numpy as np
import pandas as pd
...
def ini_block(row, ini, stream, run_tag_extra=''):
    ...
    run_tag = f"{RUN_TAG}{run_tag_extra}"
...
def smoke_lines(stream):
    return f"""cat > /tmp/epslowa_smoke_{stream}.ini <<'EOINI'
k_mode = 2
alpha_YM = 1.0
V0 = 0.05
perturb_amp = 0.001
...
kz_suppress_hi = 14
target_tu = 2
run_tag = smokelowa{stream}_nx1152
EOINI
rm -rf $WDIR/outputs/ym_k2_a1.000*smokelowa{stream}*
(cd $WDIR/outputs && $WDIR/ym_coupled /tmp/epslowa_smoke_{stream}.ini >> $LOG 2>&1)
(cd $WDIR/outputs && python3 ../analysis/remote_timeseries.py "ym_k2_a1.000*smokelowa{stream}*" >> $LOG 2>&1)
...
def build_stream_lines(stream, pts):
    """Lines for one stream's runs (smoke test + all its ini blocks). Caller
    supplies $WDIR/$LOG in scope (either top-level vars or function-local)."""
    lines = [f'echo "=== epsscan_lowalpha {stream} start $(date) ===" >> $LOG',
             smoke_lines(stream)]
    for i, (_, r) in enumerate(pts.iterrows()):
        ini = f"/tmp/epslowa_{stream}_{i}_a{r['alpha']}_v{r['V0']}_eps{r['EPS']}_kz{r['kz']}.ini"
...
def emit_single(stream, wdir, pts, path):
    lines = ["#!/bin/bash",
             f"# EPS-scan low-alpha extension -- {stream}: {len(pts)} runs (2026-07-20)",
             f"WDIR={wdir}", "mkdir -p $WDIR/logs $WDIR/outputs",
             f"LOG=$WDIR/logs/epsscan_lowalpha_{stream}_progress.log"]
...
def emit_abi(assign, path):
    lines = ["#!/bin/bash", "# EPS-scan low-alpha extension -- abi (3 GPUs, 2026-07-20)",
    ...
    for g in range(3):
        ...
        lines.append(f"    LOG=$WDIR/logs/epsscan_lowalpha_{stream}_progress.log")
        ...
        lines.append("}")
        lines.append("")
    lines.append('echo "=== abi: launching 3-GPU epsscan_lowalpha campaign $(date) ==="')
    for g in range(3):
...
def main():
    gb_path = os.path.join(SWEEP_DIR, 'eps_collapse_gridB_lowalpha.csv')
    if not os.path.exists(gb_path):
        print(f"ERROR: {gb_path} not found -- run gen_eps_collapse_gridB_lowalpha.py first",
              file=sys.stderr)
        sys.exit(1)
    ...
    for idx in allpts.sort_values('cost', ascending=False).index:
        ...
        for name in ALL_STREAM_NAMES:
            cost = allpts.at[idx, 'cost'] * (G.ABI_SLOWDOWN if STREAM_CLASS[name] == '1080ti' else 1.0)
            t = finish[name] + cost
            if best_t is None or t < best_t:
                best, best_t = name, t
        finish[best] += allpts.at[idx, 'cost'] * (G.ABI_SLOWDOWN if STREAM_CLASS[best] == '1080ti' else 1.0)
        assign[idx] = best
    ...
    allpts.to_csv(os.path.join(SWEEP_DIR, 'epsscan_lowalpha_manifest.csv'), index=False)
    for name, _, wdir in SINGLE_STREAMS:
        emit_single(name, wdir, allpts[allpts['stream'] == name],
                    os.path.join(SCRIPT_DIR, f'epsscan_lowalpha_{name}.sh'))
    emit_abi({f'abi{g}': allpts[allpts['stream'] == f'abi{g}'] for g in range(3)},
             os.path.join(SCRIPT_DIR, 'epsscan_lowalpha_abi.sh'))
    print(f"\nWritten scripts/epsscan_lowalpha_{{t126,t133,t136,t140,abi}}.sh + "
          f"sweep/epsscan_lowalpha_manifest.csv")
