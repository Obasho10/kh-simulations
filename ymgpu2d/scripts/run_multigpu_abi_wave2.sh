#!/bin/bash
# Auto-generated multi-GPU campaign: wave2
# 30 (α,V0) combos, 205 kz runs across 3 GPUs
# Launch: nohup bash scripts/run_multigpu_abi_wave2.sh > logs/multigpu_wave2.log 2>&1 &
...
EOINI
    ...
    # α=1.0 V0=0.1
    cat > /tmp/g0_a1.0_v0.1_k5.ini <<'EOINI'
k_mode = 5
alpha_YM = 1.0
V0 = 0.1
perturb_amp = 0.001
...
target_tu = 100
EOINI
    echo "[GPU0] a=1.0 V0=0.1 kz=5 start $(date)" >> $LOG
    cd $WDIR && ./ym_coupled /tmp/g0_a1.0_v0.1_k5.ini >> $LOG 2>&1
    echo "[GPU0] a=1.0 V0=0.1 kz=5 done $(date)" >> $LOG
    cat > /tmp/g0_a1.0_v0.1_k6.ini <<'EOINI'
k_mode = 6
alpha_YM = 1.0
V0 = 0.1
perturb_amp = 0.001
...
target_tu = 100
EOINI
    echo "[GPU0] a=1.0 V0=0.1 kz=6 start $(date)" >> $LOG
    cd $WDIR && ./ym_coupled /tmp/g0_a1.0_v0.1_k6.ini >> $LOG 2>&1
    echo "[GPU0] a=1.0 V0=0.1 kz=6 done $(date)" >> $LOG
...
echo "=== wave2: launching 3-GPU campaign $(date) ==="
run_gpu0 &
PID0=$!
run_gpu1 &
PID1=$!
