...
mode region in production runs.
...
color-charge densities $Q^a_s$. The gauge is $A_x^a=A_y^a=0$ with only
$A_z^a$ retained ($\partial_y\equiv0$, no $y$-components of $E$ or of the
fluid velocity are excited by the dynamics below).
...
The remaining contributions are the two commutator terms,
\begin{equation}
...
\end{equation}
Write both as scalar triple products in color space,
$[\vec X,\vec Y,\vec Z]\equiv\epsilon^{abc}X^aY^bZ^c$. The triple product
is odd under exchanging its first and third arguments, so
$[\vec B_y,\vec A_z,\vec E_x]=-[\vec E_x,\vec A_z,\vec B_y]$ and
\begin{equation}
\frac{\dd E}{\dd t}\bigg|_{\mathrm{NA}}
...
With $\vec A_z=(\Az,0,0)$ plus small wave components, the leading term of
\eqref{eq:battery} is
\begin{equation}
\frac{\dd E}{\dd t}\bigg|_{\mathrm{NA}}
...
\label{eq:battery23}
\end{equation}
where $b=B_y^2+\ii B_y^3$ and $e_x=E_x^2+\ii E_x^3$ are the circular pairs
of \S\ref{sec:sixfield}. For any mode obeying the linearized Amp\`ere-$x$
relation $\gamma e_x=-\ii\OmA b$ (eq.~\eqref{eq:lin-ex} below),
$\Imy(b^*e_x)=-\OmA|b|^2/\gamma$, so
\begin{equation}
\frac{\dd E}{\dd t}\bigg|_{\mathrm{NA}}
...
cross product with the color-1 background, and for any color vector
$\vec X=(X^1,X^2,X^3)$ the 2/3-components of $\vec A_z\times\vec X$ with
$\vec A_z=(\Az,0,0)$ background are
\begin{equation}
(\vec A_z\times\vec X)^2=-\Az X^3,\qquad
(\vec A_z\times\vec X)^3=+\Az X^2
\quad\Longrightarrow\quad
(\vec A_z\times\vec X)^2+\ii(\vec A_z\times\vec X)^3=\ii\,\Az\,X^c ,
\label{eq:crosscomplex}
\end{equation}
where $X^c=X^2+\ii X^3$. (Contributions with the perturbed $a$ against
...
Linearize \eqref{eq:amperex} for colors 2,3 and combine as
$(2)+\ii(3)$. The curl term gives $-\ii\kz b$. The commutator, by
\eqref{eq:crosscomplex} with $\vec X=\vec B_y$, gives
$-\alpha\,\ii\Az\,b$. The current perturbation vanishes: by
\eqref{eq:currents}, $\delta J_x^{2,3}$ needs either a background
$v_x$ (zero) or a product of two perturbations ($Q^{2,3}_s$ and
$\delta v_{xs}$). Hence
\begin{equation}
...
\begin{step}[Faraday]
Linearize \eqref{eq:faraday}: $\partial_xE_z\to e_z'$,
$-\partial_zE_x\to-\ii\kz e_x$, and the commutator
$+\alpha(\vec A_z\times\vec E_x)\to+\ii\alpha\Az e_x$:
\begin{equation}
\gamma\,b=-\ii\kz e_x+\ii\alpha\Az e_x+e_z'
=-\ii\,\OmF\,e_x+e_z' .
\label{eq:lin-b}
\end{equation}
Note the opposite relative sign of the two $\alpha\Az$ shifts in
\eqref{eq:lin-ex} vs \eqref{eq:lin-b}: Amp\`ere-$x$ sees $\OmA$, Faraday
sees $\OmF$. Their product, not either alone, is what enters the local
dispersion relation (\S\ref{sec:local}); the split originates in the model
keeping $A_x=0$ fixed while $E_x\neq0$, and is inherited from the two
distinct covariant structures in \texttt{khaxn.pdf} (cf.\ its eqs.\ (2)/(3)
vs (16)/(19)).
\end{step}

\begin{step}[Amp\`ere-$z$]
Linearize \eqref{eq:amperez}: no commutator (\S\ref{ssec:modeleqs}), curl
term $\partial_xB_y\to b'$ (backward difference in the code; see
\S\ref{ssec:discrete}). The current perturbation now survives at first
order because the beams carry background $v_z=\pm v$:
\begin{equation}
\delta J_z^{2,3}
=-\Bigl[n_A\,\delta Q^{2,3}_A v_{zA}+n_B\,\delta Q^{2,3}_B v_{zB}\Bigr]
=-v\bigl(\delta Q_A^{2,3}-\delta Q_B^{2,3}\bigr),
\end{equation}
using $n_A=n_B=1$, $v_{zA}=-v_{zB}=v$ (density and velocity perturbations
multiply the \emph{zero} background $Q^{2,3}$, hence drop). In circular
form $\delta J_z^c=-v(q_A-q_B)$ and
\begin{equation}
...
Directly from \eqref{eq:potential}:
\begin{equation}
\gamma\,a=-e_z .
\label{eq:lin-a}
\end{equation}
...
Take \eqref{eq:precession} in advective form for beam A
($Q^1_A=1$ background, $v_{zA}=v$). The advection term contributes the
Doppler shift $\ii\kz v\,q_A$; the source needs the 2- and 3-components of
$\vec Q\times\vec A_z$ keeping first-order terms. With
$\vec Q=(1,Q^2,Q^3)$ and $\vec A_z=(\Az,a_2,a_3)$:
\begin{align}
...
Combine as $(2)+\ii(3)$:
\begin{equation}
(\vec Q\times\vec A_z)^c
=Q^3\Az-a_3+\ii a_2-\ii Q^2\Az
=\ii\,(a_2+\ii a_3)-\ii\Az\,(Q^2+\ii Q^3)
=\ii\,a-\ii\,\Az\,q_A .
\end{equation}
Hence $\gamma q_A+\ii\kz v\,q_A=\alpha v(\ii a-\ii\Az q_A)$, i.e.
\begin{equation}
\gamma\,q_A=\ii\alpha v\,a-\ii v\,(\kz+\alpha\Az)\,q_A
=\ii\alpha v\,a-\ii v\,\OmA\,q_A .
\label{eq:lin-qA}
\end{equation}
The combination is meaningful: the beam's Doppler shift $\kz v$ and its
precession frequency $\alpha v\Az$ enter only through the single product
$v\,\OmA$.
\end{step}

\begin{step}[Precession, beam B]
Repeat with $Q^1_B=-1$, $v_{zB}=-v$:
\begin{align}
(\vec Q\times\vec A_z)^2&=Q^3\Az+a_3,\qquad
(\vec Q\times\vec A_z)^3=-a_2-Q^2\Az ,\\
(\vec Q\times\vec A_z)^c
&=Q^3\Az+a_3-\ii a_2-\ii Q^2\Az
=-\ii\,a-\ii\Az\,q_B .
\end{align}
Then $\gamma q_B-\ii\kz v q_B=\alpha(-v)(-\ii a-\ii\Az q_B)$, i.e.
\begin{equation}
...
of \eqref{eq:khaxn23} is absent), and
\item no background $B_{y1}$ coupling in Amp\`ere-$x$ and no fluid
...
simulation-vs-solver agreement; the $a_y^\pm$ sector of the given theory
decouples at this order and is not simulated).
\end{enumerate}
...
does not merely reduce the outer branch --- it removes it. Measured global
...
In the saturated region $|\xi|\gtrsim2$: $v=\pm\Vo$ and
$u=\alpha\Vo(|\xi|-\ln2)$ by \eqref{eq:usat}. On the driven side
($v^3\OmA<0$; concretely $\xi<0$ for the $2+\ii3$ polarization when
$\OmA>0$) the magnitude of the drive is
\begin{equation}
...
\label{eq:Gdef}
\end{equation}
--- a function of position only through the local covariant wavenumber
$w$, which decreases linearly outward from $\kz$ at the layer.
...
resonance point $\hat w=1$ (i.e.\ $w=\Kwn$, $\gamma/\Vo=\Kwn$).
\end{step}
...
The well is shallow: the branch holds \emph{less than a quarter
wavelength} of action at production parameters (measured
$S=0.31\pi$--$0.46\pi$), so naive Bohr--Sommerfeld $(n+\tfrac12)\pi$
quantization fails and the connection conditions at the two edges do all
the work.

\subsection{Geometry of the well}
\label{ssec:wellgeom}

Fix the driven side ($\xi<0$ for the $2+\ii3$ pair). Moving inward from
the wall:
\begin{itemize}
\item \textbf{Outer edge}: the window radius $\xi_w$ (hard cut $\to$
zero-flux/Neumann face for $a$, \S\ref{ssec:discrete}); or, if the window
is loose, the free turning point where $G$ falls back to $\gamma^2$
(at $w=\hat w_-$ side, eq.~\eqref{eq:welledges}).
\item \textbf{Inner edge}: the ``knee'' $\xi_0$ where the $v^3$ factor
switches the drive off over $\lesssim1$ $\xi$-unit. Since
...
\item \textbf{Interior}: $Q>0$ from \eqref{eq:Qexact}. In the
window-limited regime relevant near the dispersion peak, $u\ll\kz$ across
the well, so $\D\approx\gamma^2+\kz^2$ and $G\approx G(\kz)$ vary by
$<5\%$ across the well --- the numerically confirmed \emph{square-well
regime}.
\item \textbf{Beyond the inner edge} ($|\xi|<\xi_0$): $g\to0$, so
$Q\to-\D\approx-(\gamma^2+\kz^2)$: evanescent with decay constant
\begin{equation}
...
distance $s$ inward from the outer wall. The Neumann face ($a'=0$ at
$s=0$) selects
\begin{equation}
a(s)=\cos(k_xs).
\end{equation}
...
At the inner edge $s=\Delta$ (well width), with total action
$S=k_x\Delta$:
\begin{equation}
...
$a\propto e^{-\kappa(s-\Delta)}$, with $a'/a=-\kappa$. The drive switches
off over a distance short compared with $2\pi/k_x$ (the ``sharp step''),
so $a$ and $a'$ are continuous across it:
\begin{equation}
-k_x\tan S=-\kappa
\qquad\Longrightarrow\qquad
...
the ceiling so that the well is exactly this shallow.
\end{step}
...
rather than the wall, plus $n\pi$ for overtones. The $\pi/4$ terms are the standard Airy
connection phases at first-order turning points (the linear-turning-point
matched asymptotics that replace the sharp-step matching of
\S\ref{ssec:connection}); the Neumann wall contributes phase $0$ (cosine
antinode at the wall, exactly as in \eqref{eq:tanS}).
...
$\sigma$-chased dominant ($n=0$) six-field eigenvalues to $\le2\%$ (median
...
With $S(\gamma)$ decreasing in $\gamma$, the roots of
$S(\gamma)=\Phi+n\pi$ form a ladder $\gamma_0>\gamma_1>\dots$ with spacing
\begin{equation}
\Delta\gamma\simeq\frac{\pi}{|\partial S/\partial\gamma|},
\label{eq:ladderspacing}
\end{equation}
a few $\times10^{-2}\,$TU$^{-1}$ at loose windows. A shift-invert
eigensolver returns the eigenvalue nearest its target $\sigma$; the
legacy default $\sigma=0.55\,\gamma_{\mathrm{WKB}}(\text{quartic})$ lands
mid-ladder wherever the quartic underestimates (low $\alpha$, or $\kz$
above the quartic's peak), silently selecting an overtone. Consequence
(referee-critical, discovered via this theory): at
$(\alpha,\Vo,\kz)=(1,0.05,4)$, window 20, the dominant mode has
$\gamma=0.134$ but default-$\sigma$ returned $0.106$ and the cached survey
curve carried $0.082$ --- an $n\approx3$ overtone; the eigenmode-seeded
simulation then faithfully grew the overtone it was seeded with
(plateau-audited $0.081$). \emph{Simulation-vs-solver agreement certifies
numerics, not dominant-mode selection.} All reference data now use
$\sigma$-chasing (re-shifting $\sigma$ upward until the found window is
clear).
% ==============================================================================
...
measured peak $4.5\to5.5$ (formula: $4.45\to5.85$).
\end{step}
...
$(\kz,\xi_{\mathrm{sponge}})$. Because the budget also shifts which $\kz$
...
coincides with \eqref{eq:xicharhighk} at $\alpha\Vo\kz\approx1.4$ and
tracks it within a few tens of percent over the campaign grid (e.g.\
$\alpha=2$, $\Vo=0.1$, $\kz=2$: code $1.58$ vs Hermite
$\approx1.5$). A width mismatch only launches a transient that decays
within a few growth times, so the approximation is operationally
