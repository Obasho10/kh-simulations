...
Specialize \eqref{eq:battery} to the production background: the color-1
component of $\vec A_z$ is the frozen $\Az(\xi)$ of \eqref{eq:Az1}, and the
color-2/3 components of $\vec A_z$ are pure wave perturbation, absent from
the background (\S\ref{sec:sixfield}). Keeping only the background piece,
$\vec A_z\approx(\Az,0,0)$ (its own wave part would make this term
higher-order than the quadratic energy already is, since $B_y,E_x$ are
themselves perturbations), only the color-1 component of the cross product
survives the dot product:
\begin{equation}
\vec A_z\cdot(\vec B_y\times\vec E_x)\approx\Az\,(\vec B_y\times\vec E_x)^1
=\Az\,\bigl(B_y^2E_x^3-B_y^3E_x^2\bigr),
\end{equation}
using the component formula $(\vec B\times\vec E)^1=B^2E^3-B^3E^2$.
Substituting into \eqref{eq:battery}:
\begin{equation}
\frac{\dd E}{\dd t}\bigg|_{\mathrm{NA}}
