...
picture}} boxes carry the intuition behind each derivation.

The document aims to be readable by a physics student with no plasma
background: every term beyond a standard bachelor curriculum, and every
piece of program-specific shorthand, is either explained at first use or
carries an explicit back-reference to where it was explained. The
central vocabulary of the whole theory --- wells, bound states,
quantization, drive, resonance, Lorentzian --- is built from
undergraduate material in one place, \S\ref{ssec:dictionary}, once the
equation it describes has been derived. All figures
...
Orientation. Equation (33) of \texttt{wkb.pdf} --- taken as given,
\S\ref{ssec:given} --- is a quantization condition of the kind met in a
first quantum-mechanics course: it fixes the mode frequency $\omega$ by
demanding that the well near the shear layer, approximated there as a
perfect parabola (the ``harmonic core'', i.e.\ the quantum harmonic
oscillator setting; cf.\ \S\ref{ssec:hermite}), hold the $n$-th
oscillator state. This section does two things with it: clears its
square roots to obtain a plain fourth-degree polynomial in $\omega$ ---
the ``WKB quartic'' quoted as $\gamma_{\mathrm{WKB}}$ in every program
table --- and evaluates the $\kz=0$ limit in closed form, which became
the single best-validated anchor of the survey era. (Where the quartic
is and is not trustworthy at finite $\kz$ is diagnosed later, in
\S\ref{sec:gap}, once the exact theory exists to compare against.)
...
elementary; for the complex (growing) roots we are about to find, one
must say once and for all \emph{which} of a complex number's two square
roots is meant --- that is all ``choice of branch'' means --- and this
choice defines the analytic continuation used everywhere in the
...
(``chromo-Weibel'') mode. The name: the Weibel instability of ordinary
plasmas is the break-up of counter-streaming currents into magnetic
filaments that reinforce the very field bunching them;
``chromo-'' marks that here the growing field is a color field. In
detail: the background $\Az$ mixes the color-2 and
...
\paragraph{Fluid sector} (each beam; advected with FCT ---
flux-corrected transport, a standard scheme for carrying quantities
along with a flow without spurious numerical ripples):
\begin{align}
&\partial_t n+\nabla\!\cdot\!(n\vec v)=0,
\label{eq:continuity}\\
...
\paragraph{Field sector} (integrated ``leapfrog'' on a Yee stagger:
the standard electromagnetic arrangement in which $E$ and $B$ components
live at mutually offset grid points and are advanced in alternating
half-steps; the offsets matter once more in \S\ref{ssec:discrete}):
\begin{align}
...
Wong's equations (S.~K.~Wong, 1970) are the classical equations of
motion of a point particle that carries a non-Abelian color charge:
the color generalization of the Lorentz-force law, together with a rule
stating that the charge vector itself rotates (``precesses'') in the
background potential --- just as a magnetic moment precesses in a
magnetic field. Only the rotation rule is needed here.
...
\label{eq:circpairs}
\end{equation}
each with the ansatz (an \emph{ansatz} is simply an assumed trial form
for the solution) $f(x,z,t)=\hat f(x)\,e^{\ii\kz z+\gamma t}$, i.e.
\begin{equation}
...
(``Helicity'' here means nothing more than which of the two circular
combinations $f^2\pm\ii f^3$ a mode lives in --- the two senses of
rotation in the color-$(2,3)$ plane.)
...
one polarization becomes \emph{tachyonic}: $\gamma^2>0$. (About the
word: ``tachyonic'' is borrowed from relativity, where a tachyon is a
hypothetical particle with imaginary mass. Operationally it means only
this: the number playing the role of $\omega^2$ in the local
oscillation law is negative, so $\omega$ is imaginary and the amplitude
grows exponentially in time instead of oscillating. Nothing here moves
faster than light.) This is the
...
The envelope \eqref{eq:gloc} is a pointwise bound; an actual mode must
have a smooth global profile, and building one costs growth. Global
tachyonic modes live in the annulus between $\xi_{\mathrm{crit}}$ and
the window radius $\xi_w$; a hard cut forces all fields to zero at the
wall (a \emph{Dirichlet} condition --- the fixed end of a string, as
opposed to the free \emph{Neumann} end met in \S\ref{ssec:discrete}).
Their rates follow from a problem formally identical to finding the
ground state of a quantum well (the well/bound-state vocabulary is
unpacked at leisure in \S\ref{ssec:dictionary}; here only the mechanics
are needed): in the local potential
$U(x)\equiv\gamma_{\mathrm{loc}}^2(x)$, increasing outward,
\begin{equation}
a''+\bigl(U(x)-\gamma^2\bigr)a=0 ,
\end{equation}
with $a=0$ at the wall $x_w$ and decay toward the layer --- the profile
oscillates where the local envelope exceeds the sought global rate
($U>\gamma^2$) and decays where it falls short.
...
$U<\gamma^2$) is $\mathrm{Ai}(\tau)$ --- the \emph{Airy function}, the
standard tabulated solution of the Schr\"odinger problem on a linear
ramp, which oscillates for $\tau<0$ and dies off for $\tau>0$.
\end{step}
...
eigenvalues of the discrete six-field operator (ARPACK: the standard
numerical library for computing a few eigenvalues of a large matrix; how
it is used --- and one way it was once misused --- is spelled out in
\S\ref{ssec:ladder}), is the backbone of the exact-action theory.
...
Orientation. The \emph{dispersion curve} is the growth rate as a
function of wavenumber, $\gamma(\kz)$, at fixed physics
$(\alpha,\Vo,\eps)$ and fixed window --- the curve every simulation
campaign measures (Fig.~\ref{fig:dispersion} shows one). Measured
curves rise with $\kz$, peak, and roll off, and the natural instinct
is to read the peak as an intrinsic preferred wavelength of the layer,
the way textbook Kelvin--Helmholtz theory would suggest. The point of
this section is that the instinct is wrong here: nothing derived so far
selects a peak (the ceiling of \S\ref{sec:ceiling} is
$\kz$-independent), and the peak the data show is set by the
\emph{window} --- the containment radius $\xi_w$ deliberately imposed
on every run (\S\ref{ssec:units}) --- through a simple geometric
crossover derived below.
...
(eigensolver, $\alpha=2$, $\Vo=0.05$, hard cut at $\xi_w=40$): $\gamma$
climbs to
...
How does the shear width $\eps$ enter, given that neither the ceiling
\eqref{eq:ceiling} nor the peak formula \eqref{eq:kzpeak} contains it?
Answer: only through the \emph{action} --- the total phase
$S=\int\sqrt{Q}\,\dd x$ held by the well, defined in the vocabulary
list of \S\ref{sec:quant}. We call this the quantization \emph{budget}
because \S\ref{sec:quant} showed the mode must pay a fixed phase toll
(the connection conditions) to exist at all; $\eps$ controls how much
phase the well can afford to accumulate. To see this cleanly we
\emph{nondimensionalize} the exact action integral --- rewrite it in
the hatted, parameter-free variables of \S\ref{ssec:natural}, so that
every dependence on $(\alpha,\Vo,\eps)$ collects into one visible
prefactor --- step by step. (This underlies the EPS-scan~v2
...
Orientation. The quartic \eqref{eq:quartic} is \S\ref{sec:quartic}'s
polynomial form of the given harmonic-core theory --- the program's
original analytic reference, quoted as $\gamma_{\mathrm{WKB}}$ in every
survey table and used for years as the comparison curve. With the exact
theory of \S\ref{sec:reduction}--\S\ref{sec:quant} now in hand, we can
state precisely where the quartic is trustworthy and where it is not,
by comparing the drive it implicitly assumes with the true drive.
...
Orientation. The \emph{two-stream instability} is the oldest
instability in plasma physics: send two streams of charge through each
other, and a small charge bunch in one stream pulls on the particles of
the other, whose regrouping reinforces the original bunch --- a
feedback loop that grows exponentially (the intuition box at the end of
this section walks through the loop). Away from the shear layer our two
beams are exactly such counter-streaming charge streams
($v_z\to\pm\Vo$), so the model unavoidably contains this instability in
the color-1 sector, on top of the shear physics being measured. Three
standard adjectives, spelled out: \emph{electrostatic} --- only the
electric field responds, no magnetic perturbation; \emph{longitudinal}
--- the perturbation varies along the same direction ($z$) as the field
and the streaming; \emph{cold} --- the beams have zero temperature, no
thermal spread of velocities (temperature is added in
\S\ref{sec:warm}). The exact cold rates in code units, derived below,
calibrate the bandpass-filter design (Appendix~\ref{app:filters}) and
were used to test (and reject) the outer two-stream as the
...
Orientation. Everything so far was \emph{linear} theory: perturbations
small enough that their squares are negligible. This section derives
the program's two nonlinear results. First, the amplitude at which the
fluid description itself gives out --- \emph{cavitation}, the wave
digging the plasma density down to zero, the same word used for the
vapor cavities behind a ship propeller. Second, what the background
$\Az$ actually does when it is allowed to evolve (the
\emph{back-reaction}), which closes the frozen-background ``battery''
caveat of \S\ref{ssec:energy}.
...
blocks; the late-time ``hard wall'' at $\Vo\gtrsim0.08$ (program
shorthand for the reproducible terminal blow-up of high-$\Vo$ runs) is
a nonlinear fluid effect. Its threshold is derived as follows.
...
Two vocabulary items. A \emph{beat} is the product of two oscillating
factors, which contains their sum and difference wavenumbers; the beat
of a mode with itself therefore contains a component with zero
wavenumber --- a $z$-independent (``DC'') part --- and, since each
factor grows as $e^{\gamma t}$, the product grows at $2\gamma$.
\emph{Ponderomotive} is the standard name for exactly this kind of
time-averaged, amplitude-squared push that a wave exerts on the medium
carrying it (the same effect that lets a focused laser trap particles).
Here: the quadratic beats of the mode grow at $2\gamma$ and accumulate
a $z$-mean density modification; the observed density-perturbation
growth at the retained $\kz$ at twice the saturating field rate is this
quadratic part taking over as the fields saturate.
\end{step}
...
(``Beat'' in the sense defined in \S\ref{ssec:cavitation}: the product
of two first-order waves.) For real fields with circular structure
...
Orientation. A fluid model is obtained from the underlying
particle-distribution picture by taking velocity \emph{moments}:
multiply the kinetic equation by $1$, $\vec v$, $v^2,\dots$ and
integrate over velocities, obtaining equations for density, momentum,
pressure, and so on. Each moment equation drags in the next one
(density needs momentum, momentum needs pressure, \dots), so the chain
must be cut by hand; a \emph{closure} is the assumed rule that cuts it.
Here we use the simplest: \emph{isothermal}, pressure $P=nT$ with $T$ a
prescribed constant temperature --- the minimal way to give the beams a
thermal spread and ask what it changes.
...
$v_{\mathrm{th}}=\sqrt T\gtrsim\Vo$ --- thermal jitter blurs the charge
bunching once the random speeds rival the streaming speed, shrinking
the unstable band \eqref{eq:tsband} --- which was the intended
validation target.
...
\emph{anisotropy} of the counter-streaming beams --- their combined
velocity distribution is two cold spikes at $\pm\Vo$ along $z$, wildly
elongated in one direction, whereas a temperature $T$ is the same in
all directions. An isotropic scalar
pressure $P=nT$ adds a restoring force only along density gradients of
the compressive response; it does not relax the directional imbalance
that powers filament growth. The scan confirmed this: across
...
Orientation. A simulation grows whatever its initial condition
contains, so to measure the growth rate of one particular mode, the run
must be started as close to that mode's exact profile as possible ---
this is \emph{seeding}. This section derives what the correct seed
profile is (\S\ref{ssec:hermite}) and dissects, with measured examples,
two distinct ways that seeding the wrong profile silently corrupts the
measurement. The closing intuition box states the single principle
behind all of it.
...
Q_0=(2n+1)\sqrt{|C_1|}
\end{equation}
($H_n$ are the Hermite polynomials of the standard oscillator solution;
all that matters below is $n=0$, where $H_0=1$ and the eigenfunction is
a pure Gaussian),
...
condition onto the eigenmode basis decides what grows first. (In plain
terms: write the initial condition as a weighted sum of the system's
eigenmodes; each component then grows at its own rate, so the winner is
decided by initial weight times growth rate, not by the experimenter's
intent.) Concretely
...
This appendix records the projection identities behind the extraction
pipeline --- the analysis step that pulls a single mode's complex
amplitude out of simulation output using a discrete Fourier transform
(DFT: the decomposition of grid data into its sinusoidal
$z$-components) --- and the sign bug those identities diagnosed.
Helicity is the circular-pair bookkeeping of \S\ref{ssec:helicity}.
...
\cup\{k_{\mathrm{mode}}+1,\dots,k_{\mathrm{hi}}\}$: an idempotent
(applying it twice does the same as once --- the defining property of a
projection) linear
...
with the FCT limiter's deposits near the Nyquist scale --- the
shortest wavelength a grid can represent, two cells.)

\paragraph{Sponge.} \texttt{kernel\_ym\_sponge} adds
$\partial_tf=-\sigma_s(\xi)f$ with
$\sigma_s=\sigma\,(|\xi|-\xi_{\mathrm{sponge}})_+$: an absorbing
potential --- a pure damping term that soaks up waves in the outer
region instead of letting them reflect, the standard ``absorbing
boundary'' trick (``non-Hermitian'' flags that it removes energy, which
no conservative term can). In the eigenproblem it appears as
