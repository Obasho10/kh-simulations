...
branch's exact local law. The third of Letter A's original gating items,
the claimed sub-$\kz=1$ two-branch structure (roadmap T1.3), is now
\textbf{closed}: eigensolver continuation plus a dedicated GPU confirmation
(\S\ref{sec:limitations}) show it is a sponge-boundary artifact, not a
genuine crossing between two physical eigenmode families — the true
dispersion in that range is smooth. Open items — the new
$V_0=0.2/\alpha\ge3.5$ failure corner, the low-$\alpha$ noise floor, and the
still-unexplained late-time \texttt{xi\_cut} failure mode — are reported
with the same weight as the successes.
\end{abstract}
...
all three; all three are now closed (\S\ref{sec:limitations}), with T1.3's
resolution being a retraction of the original claim (sponge artifact, not a
second physical branch) rather than a confirmation of it.
...
$\kz^{\rm peak}=4$ at $\alpha=1$ to $\kz^{\rm peak}=5$ at $\alpha=4$.
...
wavenumber climbs a full unit in $\kz$ as the coupling $\alpha$ is turned
up — where classical KH would pin every curve's peak at the same
width-set $\kz$. That
...
\subsection{T1.3: the sub-$\kz=1$ branch structure --- resolved as a
sponge-boundary artifact (2026-07-21)}
...
eigenmode families. The raw sweep tables at those points were produced by
the buggy pre-2026-07-17 pipeline (\S\ref{sec:pipeline}) and could not be
certified from that data alone, so the claim was re-derived independently
of the old pipeline: a fine (\(\Delta\kz=0.05\)) eigensolver continuation
(\texttt{analysis/t13\_branch\_continuation.py}) tracking several leading
eigenvalues at three sponge radii, cross-checked with the project's
standard $\sigma$-chase.

\textbf{Finding: the claim is real as a \emph{measurement} but not as
\emph{physics}.} At $\xi_{\rm sponge}=13$ (the original setting) the
solver reproduces $0.320$/$0.060$ almost exactly, but the responsible
eigenfunction is pinned near or at the sponge radius itself
($\xi_{\rm peak}\approx10$--$13$; at $\xi_{\rm sponge}=8$ it locks to
$\xi_{\rm peak}=-8.181$, the exact wall, for every $\kz\ge0.85$), its
$A_z/B_y$ amplitude ratio blows up from $\sim\!2.6$ to $40$--$150$, and its
parity correlation collapses from $\sim\!0.99$ (coherent) to $<0.3$
(incoherent) precisely where the ``dip'' occurs --- the signature of a
sponge-supported quasi-mode, not a second confinement branch of the
physical (unbounded) system. A decisive control --- a hard $\xi_{\rm
cut}=5$ Dirichlet wall placed well inside any sponge, excluding the wall
region entirely --- gives a smooth, monotonically rising, artifact-free
$\gamma(\kz)$ with no dip and no crossing (\(0.0170\), \(0.0595\),
\(0.0839\), \(0.1014\), \(0.1144\), \(0.1245\) for
$\kz=0.25,\ldots,1.50$), consistent with the T1.2 exact-action result
that the true shear branch has no interior $\kz$ maximum.

This was then confirmed in full nonlinear simulation
(\texttt{scripts/run\_campaign\_t13confirm.sh}, same $\alpha,\Vo,\eps$):
six $\xi_{\rm cut}=5$ points matched the eigensolver to $1$--$2\%$ at every
$\kz\ge0.5$ (the tightest GPU-vs-eigensolver agreement obtained anywhere in
this program), smooth and monotonic throughout; two points reproducing the
original $\xi_{\rm sponge}=13$ configuration reproduced the $0.32$/$0.06$
numbers almost exactly in the full CUDA code (not just the linear solver)
--- the $\kz=0.5$ leg overshot to $E/E_0\approx7.8\times10^{6}$ by
$t\approx36$ (consistent with $\gamma\approx0.32$), the $\kz=0.75$ leg
halted far more mildly ($E/E_0=111$ at $t\approx55$, consistent with
$\gamma\approx0.06$--$0.07$) --- proving the artifact lives in the
simulation's sponge kernel itself, not only in the linear model.
\textbf{T1.3 is closed}: the sub-$\kz=1$ dispersion is smooth, with no
genuine branch crossing; the original numbers were a real but
boundary-condition-contaminated measurement. Full writeup and table:
\texttt{FINDINGS.md} ``T1.3 CLOSED (2026-07-21)''.
...
T1.3 & Sub-$\kz=1$ branch structure & \textbf{\checkmark} Closed &
Eigensolver continuation + $\xi_{\rm cut}=5$ control + GPU confirmation
(\S\ref{sec:limitations}): the original 0.32/0.06 numbers are real
measurements of a sponge-boundary artifact, not a second physical branch;
the true dispersion is smooth (GPU matches artifact-free eigensolver
prediction to 1--2\% at every $\kz\ge0.5$)\\
\bottomrule
\end{tabular}
\end{center}

\begin{physmeaning}
All three of Letter A's explicit blockers are now closed. The theory
(T1.2) matches the exact-action model to $\le2\%$ against the
$\sigma$-chased eigensolver; the shear-width independence (T1.1) is
confirmed directly on GPU data across an order of magnitude in $\eps$ and
down to $\alpha=0.2$; and the sub-$\kz=1$ branch question (T1.3) is
resolved by retraction --- the striking original numbers are a real
measurement, but of a sponge-wall quasi-mode, not of a genuine second
eigenmode family, so Letter A's text should state the dispersion is smooth
through $\kz<1$ rather than claim any branch-crossing structure there.
Everything else in \S\ref{sec:limitations} (the new
$\Vo=0.2/\alpha\ge3.5$ corner, the noise floor, the late-onset
catastrophe, the warm closure) sits outside Letter A's three gating items
and belongs to the broader program's honesty record rather than to this
specific submission's blocking path.
\end{physmeaning}
